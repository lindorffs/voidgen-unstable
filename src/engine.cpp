...
int lua_render_subtexture(lua_State *lua_instance) {
	...
	gEngine->render_subtexture(id.c_str(), x, y, sx, sy, centered, r);
...
int lua_imgui_input_text(lua_State *lua_instance) {
	const char *title = lua_tolstring(lua_instance, 1, NULL);
	const int storage_pos = lua_tonumber(lua_instance, 2);
	ImGui::InputText(title, gEngine->input_texts[storage_pos], IM_ARRAYSIZE(gEngine->input_texts[storage_pos]), NULL);
	lua_pushlstring(lua_instance, (const char*)gEngine->input_texts[storage_pos], (size_t)strlen(gEngine->input_texts[storage_pos]));
	return 1;
}

int lua_imgui_custom_cursor(lua_State *lua_instance) {
	const char *id = lua_tolstring(lua_instance, 1, NULL);
	...
	for (int i = 0; i < MAX_TEXTURES; i++) {
		if (gEngine->Textures[i] == NULL) {
		...
		} else if (gEngine->Textures[i]->id == std::string(id)) {
			...
			b.x += 16;
			b.y += 24;
			ImGui::GetForegroundDrawList()->AddImage((void *)gEngine->Textures[i]->data, a, b, ImVec2(0, 0), ImVec2(1, 1));
			break;
		} else {
		}
	}
...
int lua_imgui_sameline(lua_State *lua_instance) {
	ImGui::SameLine();
	return 0;
}
...
int engine::initialize() {
	...
	if ( this->initialized ) {
		...
		this->window = SDL_CreateWindow( "void.GEN 2.1", this->screen_width, this->screen_height, SDL_WINDOW_HIDDEN | SDL_WINDOW_RESIZABLE);
		if( this->window == NULL )
	...
	if (this->initialized) {
		...
		luaL_requiref(this->lua_instance, "socket.core", luaopen_socket_core, 0);
		/* Removes the copy of the module from the stack. */
		lua_pop(this->lua_instance, 1);
		
		luaL_requiref(this->lua_instance, "mime.core", luaopen_mime_core, 0);
		/* Removes the copy of the module from the stack. */
		lua_pop(this->lua_instance, 1);
		
		luaL_requiref(this->lua_instance, "ssl.core", luaopen_ssl_core, 0);
		/* Removes the copy of the module from the stack. */
		lua_pop(this->lua_instance, 1);
		
		luaL_requiref(this->lua_instance, "ssl.context", luaopen_ssl_context, 0);
		/* Removes the copy of the module from the stack. */
		lua_pop(this->lua_instance, 1);
		
		luaL_requiref(this->lua_instance, "ssl.x509", luaopen_ssl_x509, 0);
		/* Removes the copy of the module from the stack. */
		lua_pop(this->lua_instance, 1);
		
		luaL_requiref(this->lua_instance, "ssl.config", luaopen_ssl_config, 0);
		/* Removes the copy of the module from the stack. */
		lua_pop(this->lua_instance, 1);
		...
		lua_register(this->lua_instance, "IMGui_sameline", lua_imgui_sameline);
		...
		lua_register(this->lua_instance, "IMGui_text_input", lua_imgui_input_text);
		lua_register(this->lua_instance, "IMGui_custom_cursor", lua_imgui_custom_cursor);
...
int engine::cleanup() {
	...
	this->state_manager->current_state->unload();
